...
public abstract class WeatherRequestTask  extends AsyncTask<String, String, TaskOutput> {
    ProgressDialog progressDialog;
    Context context;
    MainActivity activity;
    public int loading = 0;

    public WeatherRequestTask(Context context, MainActivity activity, ProgressDialog progressDialog) {
    ...
    protected void onPreExecute() {
        ...
        if (!progressDialog.isShowing()) {
            progressDialog.setMessage(context.getString(R.string.downloading_weater));
        ...
        switch (output.taskResult) {
            case SUCCESS: {
                ...
                if (ParseResult.CITY_NOT_FOUND.equals(parseResult)) {
                    ...
                    Snackbar.make(activity.findViewById(android.R.id.content), context.getString(R.string.msg_err_json), Snackbar.LENGTH_LONG).show();
                }
                break;
            }
            ...
            case BAD_RESPONSE: {
                Snackbar.make(activity.findViewById(android.R.id.content), context.getString(R.string.msg_connection_error), Snackbar.LENGTH_LONG).show();
                break;
            }
            case IO_EXCEPTION: {
    ...
    private URL provideURL(String[] coords) throws UnsupportedEncodingException, MalformedURLException {
        ...
        if (coords.length == 2) {
            ...
            //get city name from shared preferences/ if empty then set to Limerick
            final String city = sp.getString("city", DefaultCity.DEFAULT_CITY);
        ...
        if(getAPIName().equals("forecast/daily")){
            //request five days forecast including current day
            urlBuilder.append("&cnt=5");
        }
